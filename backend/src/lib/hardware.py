...
class Tubes:
    ...
    flags = (0b1000, 0b0000, 0b1001, 0b0001, 0b1110, 0b0110, 0b1010, 0b0010, 0b1100, 0b0100)
    tubes = (L0_H1, L1_H0, L2_M1, L3_M0, L4_S1, L5_S0)
    digits = [0, 0, 0, 0, 0, 0]
    cmp_time = [0, 0, 0]
    ...
    @classmethod
    def _set_digit(cls, digit):
        
        flag = cls.flags[digit]
    ...
    def _get_digits(cls):
        ...
        for n, number in enumerate(cls._RTC.datetime()[4:-1]):
            digits[2 * n], digits[2 * n + 1] = number // 10, number % 10


    @classmethod
    def _set_tube_digit(cls, tube, digit):

            cls._set_digit(digit)
            tube.value(1)
            gc.collect()
            tube.value(0)
            time.sleep_us(1050)
    ...
    def _scrolling(cls):
        tubes = cls.tubes
        set_tube_digit = cls._set_tube_digit
        _digits = list(range(10))
        time_digits = cls.digits

        for i in range(25):
            digits = _digits[:]
            random_shuffle(digits)

            if i >= 20:
                digits[0:(i - 19)] = time_digits[0:(i - 19)]

            start_time = time.ticks_ms()
            while time.ticks_diff(time.ticks_ms(), start_time) <= 30 + (10 ** (i / 5)) / 650:
                for tube, digit in zip(tubes, digits):
                    set_tube_digit(tube, digit)
    ...
    @classmethod 
    def show_time(cls):
        # HZ should be measured for prod
        cls._get_digits()

        if cls.digits[3:] == cls.cmp_time:
            cls._scrolling()

        for tube, digit in zip(cls.tubes, cls.digits):
            cls._set_tube_digit(tube, digit)
...
class DS1307(object):
    ...
    def __init__(self, addr=0x68):
        self.i2c = I2C(scl=Pin(22), sda=Pin(21), freq=32768)
        self.addr = addr
