...
class Lamp:
    ...
    number_dict = {0: 0b1000, 1: 0b0000, 2: 0b1001, 3: 0b0001, 4: 0b1110, 5: 0b0110, 6: 0b1010, 7: 0b0010, 8: 0b1100, 9: 0b0100}
    lamp_tuple = (L0_H1, L1_H0, L2_M1, L3_M0, L4_S1, L5_S0)
    ...
    def _scrolling_numbers(cls, time_now):
        for z in range(25):
                ...
                if z >= 19:
                    number_list[0:(z-18)] = time_now[0:(z-18)]

                maped_time = lambda number_list: zip(cls.lamp_tuple, number_list)
                start_time = time.ticks_ms()
                #30 + (10 ** (z / 12)) * 2
                #30 + (10 ** (z / 5)) / 650
                while time.ticks_diff(time.ticks_ms(), start_time) <= 30 + (10 ** (z / 5)) / 650:
                    
                    for lamp, number in maped_time(number_list):
                        if z == 24 and lamp is cls.L5_S0:
                            number = cls._RTC.datetime()[6] % 10 - 1
    ...
    def _time_now(cls):
        ...
        time_now = (( x // 10, x % 10) for x in cls._RTC.datetime()[4:-1])
        time_now = (element for tupl in time_now for element in tupl)

        return tuple(time_now)
    ...
    def show_time(cls, scrolling=True):
        ...
        #time_now[3:] == (0, 0, 0)
        #time_now[5]
        if  time_now[3:] == (0, 0, 0) and scrolling:
            cls._scrolling_numbers(time_now)

        maped_time = zip(cls.lamp_tuple, time_now)
...
if __name__ == "__main__":
    Lamp.set_date()
    Lamp.on()
    # should work like callback to event loop
    while True:
        Lamp.show_time()